...
This is a **breaking** release: the public API surface was deliberately narrowed, so external consumers may need to adjust (see Changed). Internals were also decomposed and hardened with no behavioural change to the supported API.
...
- `ConfigureAwait(false)` on all library awaits; `static ReadOnlySpan<byte>` markers, a `FrozenDictionary` lookup, and a right-sized copy buffer; expanded unit coverage of the brute-force helpers and direct SRR reconstruction.
